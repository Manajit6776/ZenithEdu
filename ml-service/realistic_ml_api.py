...
class FeatureEngineer:
    ...
    def create_advanced_features(df: pd.DataFrame) -> pd.DataFrame:
        ...
        avg_score = df_features['avg_assignment_score'] if 'avg_assignment_score' in df_features.columns else \
                    df_features['avg_submission_score'] if 'avg_submission_score' in df_features.columns else pd.Series(65, index=df_features.index)
        
        df_features['academic_score'] = (
            df_features['cgpa'] * 0.4 +
            (avg_score / 100) * 100 * 0.3 +
        ...
        total_subs = df_features['total_submissions'] if 'total_submissions' in df_features.columns else \
                     (df_features['assignment_completion_rate'] * 10) if 'assignment_completion_rate' in df_features.columns else pd.Series(5, index=df_features.index)
...
class EnhancedStudentPerformancePredictor:
    ...
    def train_enhanced_ensemble(self, data: pd.DataFrame) -> Dict:
        ...
        if best_model:
            ...
            # Store feature importance from the best model before calibration
            current_model = self.models[best_model]
            if hasattr(current_model, 'feature_importances_'):
                self.feature_importance = current_model.feature_importances_
            elif hasattr(current_model, 'coef_'):
                self.feature_importance = np.abs(current_model.coef_).mean(axis=0)
        ...
        self.model_metadata['metrics'] = model_results.get(best_model, {})

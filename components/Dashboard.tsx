...
const CARD_THEMES: Record<string, { iconBg: string; glow: string; border: string; cardBg: string; cardBgLight: string; iconColor: string; iconColorLight: string; borderLight: string }> = {
  indigo: { iconBg: 'bg-indigo-500/20', glow: 'bg-indigo-500', border: 'border-indigo-500/20', borderLight: 'border-indigo-100', cardBg: 'rgba(99,102,241,0.06)', cardBgLight: 'rgba(99,102,241,0.03)', iconColor: '', iconColorLight: '' },
  emerald: { iconBg: 'bg-emerald-500/20', glow: 'bg-emerald-500', border: 'border-emerald-500/20', borderLight: 'border-emerald-100', cardBg: 'rgba(16,185,129,0.06)', cardBgLight: 'rgba(16,185,129,0.03)', iconColor: '', iconColorLight: '' },
  amber: { iconBg: 'bg-amber-500/20', glow: 'bg-amber-500', border: 'border-amber-500/20', borderLight: 'border-amber-100', cardBg: 'rgba(245,158,11,0.06)', cardBgLight: 'rgba(245,158,11,0.03)', iconColor: '', iconColorLight: '' },
  rose: { iconBg: 'bg-rose-500/20', glow: 'bg-rose-500', border: 'border-rose-500/20', borderLight: 'border-rose-100', cardBg: 'rgba(244,63,94,0.06)', cardBgLight: 'rgba(244,63,94,0.03)', iconColor: '', iconColorLight: '' },
};
    ...
    >
      ...
      <div className="relative z-10 flex items-start justify-between">
        <div>
          ...
          <p className={`text-xs font-semibold  tracking-widest ${isDark ? '' : ''} mb-1`}>{title}</p>
          <h3 className={`text-3xl font-black ${isDark ? 'text-white' : ''} tracking-tight`}>{displayValue}</h3>
          {subtitle && <p className={`text-xs ${isDark ? '' : ''} mt-1`}>{subtitle}</p>}
        </div>
        <div className="flex flex-col items-end gap-2">
          {trend && (
            <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${trend.isPositive ? 'bg-emerald-500/15 ' : 'bg-rose-500/15 '
              }`}>
        ...
        <div className="flex items-center gap-3">
          <div>
            <div className="flex items-center gap-2">
              <h3 className={`text-base font-bold ${isDark ? 'text-white' : ''}`}>{title}</h3>
              {badge && (
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isDark ? 'bg-indigo-500/20  border-indigo-500/30' : 'bg-indigo-50  border-indigo-100'} border`}>
                  {badge}
                </span>
              )}
            </div>
            {subtitle && <p className={`text-xs ${isDark ? '' : ''} mt-0.5`}>{subtitle}</p>}
          </div>
        </div>
        {action}
      </div>
...
const TopPerformerCard: React.FC<{ student: Student; index: number; key?: string }> = ({ student, index }) => {
  const { isDark } = useTheme();
  const rankConfig = {
    1: { icon: <Crown className="w-4 h-4" />, bg: isDark ? 'from-amber-500/20 to-yellow-500/10' : 'from-amber-50 to-yellow-50', border: isDark ? 'border-amber-500/30' : 'border-amber-200', ring: 'ring-amber-500/40', bar: 'from-amber-400 to-yellow-400', label: isDark ? '' : '' },
    2: { icon: <Medal className="w-4 h-4" />, bg: isDark ? 'from-slate-400/20 to-slate-500/10' : 'from-slate-50 to-slate-100', border: isDark ? 'border-slate-400/30' : 'border-slate-200', ring: 'ring-slate-400/30', bar: 'from-slate-300 to-slate-400', label: isDark ? '' : '' },
  ...
  const cfg = rankConfig[student.rank as 1 | 2 | 3] || { icon: null, bg: isDark ? 'from-indigo-500/10 to-purple-500/5' : 'from-indigo-50 to-purple-50', border: isDark ? 'border-white/10' : 'border-slate-200', ring: 'ring-indigo-500/20', bar: 'from-indigo-500 to-purple-500', label: isDark ? '' : '' };
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="min-w-0">
          <h4 className={`font-bold ${isDark ? 'text-white' : ''} text-base truncate group-hover: dark:group-hover: transition-colors`}>{student.name}</h4>
          <p className={`text-xs ${isDark ? '' : ''}`}>{student.department}</p>
          <div className="flex items-center gap-3 mt-1.5">
            ...
            <span className={`text-xs ${isDark ? '' : ''} font-semibold`}>{student.attendance}% att.</span>
          </div>
        </div>
      </div>
      ...
      {/* CGPA Bar */}
      <div className="mt-2">
        <div className={`flex justify-between text-[10px] mb-1.5 ${isDark ? '' : ''}`}>
      ...
      {student.achievements && (
        <div className="mt-3 flex flex-wrap gap-1">
          {student.achievements.slice(0, 2).map((a, i) => (
            <span key={i} className={`text-[9px] px-1.5 py-0.5 rounded-full border ${isDark ? 'border-white/10  bg-white/5' : 'border-slate-200  bg-slate-50'}`}>
              {a}
            </span>
          ))}
          {student.achievements.length > 2 && (
            <span className="text-[9px] px-1.5 py-0.5 rounded-full">+{student.achievements.length - 2}</span>
          )}
        </div>
      )}
    </motion.div>
          ...
          >
            <div className={`flex items-center gap-3 p-4 border-b ${isDark ? 'border-white/8' : 'border-slate-100'}`}>
              ...
              <input
                ...
                className={`flex-1 bg-transparent ${isDark ? 'text-white' : ''} text-sm outline-none placeholder-slate-600`}
              />
              <kbd className={`text-[10px] ${isDark ? ' border-white/10' : ' border-slate-200'} border rounded px-1.5 py-0.5`}>ESC</kbd>
            </div>

            {query === '' ? (
              <div className="p-4">
                <p className={`text-[10px] ${isDark ? '' : ''}  tracking-wider mb-3 font-semibold`}>Quick actions</p>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { icon: <UserCircle className="w-4 h-4" />, label: 'Add New Student', to: '/students' },
                    { icon: <DollarSign className="w-4 h-4" />, label: 'Record Fee Payment', to: '/fees' },
                    { icon: <Calendar className="w-4 h-4" />, label: 'Schedule Event', to: '/timetable' },
                    { icon: <Mail className="w-4 h-4" />, label: 'Send Notice', to: '/notices' },
                  ].map((a, i) => (
                    <button 
                      key={i} 
                      onClick={() => { navigate(a.to); onClose(); }}
                      className={`flex items-center gap-2.5 p-3 rounded-xl ${isDark ? ' hover:text-white hover:bg-white/8 border-transparent hover:border-white/8' : ' hover: hover:bg-slate-50 border-transparent hover:border-slate-200'} transition-all text-sm text-left border`}
                    >
                      <span className="dark:">{a.icon}</span>
                      {a.label}
                    </button>
                  ))}
                </div>
              ...
              <div className="p-2 max-h-80 overflow-y-auto">
                {filtered.length > 0 ? (
                  <>
                    <p className={`text-[10px] ${isDark ? '' : ''}  tracking-wider px-3 py-2 font-semibold`}>Students</p>
                    {filtered.map((s, i) => (
                      <motion.div key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl ${isDark ? 'hover:bg-white/8 text-white' : 'hover:bg-slate-50 '} transition-all cursor-pointer group`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-sm font-medium ${isDark ? 'text-white' : ''} truncate`}>{s.name}</p>
                          <p className={`text-xs ${isDark ? '' : ''}`}>{s.department} · CGPA {s.cgpa}</p>
                        </div>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${s.status === 'Active' ? ' bg-emerald-500/10' : (isDark ? ' bg-white/5' : ' bg-slate-100')}`}>
                          {s.status}
                        </span>
                      </motion.div>
                    ))}
                  ...
                  <div className={`py-8 text-center ${isDark ? '' : ''} text-sm`}>No results found</div>
                )}
              </div>
            )}

            <div className={`flex items-center gap-4 px-4 py-3 border-t ${isDark ? 'border-white/5' : 'border-slate-100'}`}>
              <span className={`flex items-center gap-1.5 text-[10px] ${isDark ? '' : ''}`}><kbd className={`${isDark ? 'border-white/10' : 'border-slate-200'} border rounded px-1 py-0.5`}>↑↓</kbd> Navigate</span>
              <span className={`flex items-center gap-1.5 text-[10px] ${isDark ? '' : ''}`}><kbd className={`${isDark ? 'border-white/10' : 'border-slate-200'} border rounded px-1 py-0.5`}>↵</kbd> Select</span>
              <span className={`flex items-center gap-1.5 text-[10px] ${isDark ? '' : ''}`}><kbd className={`${isDark ? 'border-white/10' : 'border-slate-200'} border rounded px-1 py-0.5`}>⌘K</kbd> Toggle</span>
            </div>
          </motion.div>
        </>
      )}
...
const LiveClock: React.FC = () => {
  ...
  return (
    <div className="text-right hidden md:block">
      <p className={`text-lg font-black ${isDark ? 'text-white' : ''} tabular-nums tracking-tight`}>{format(time, 'HH:mm:ss')}</p>
      <p className={`text-[10px] ${isDark ? '' : ''}`}>{format(time, 'EEEE, MMMM d yyyy')}</p>
    </div>
  );
};
...
const ChartTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className={`rounded-xl border ${isDark ? 'border-white/10' : 'border-slate-200'} p-3 text-sm`} 
      ...
      <p className={`${isDark ? '' : ''} text-xs mb-2 font-semibold`}>{label}</p>
      {payload.map((p: any, i: number) => (
        <div key={i} className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full" style={{ backgroundColor: p.color }} />
          <span className={`${isDark ? '' : ''} text-xs capitalize`}>{p.name}:</span>
          <span className={`${isDark ? 'text-white' : ''} font-bold text-xs`}>{typeof p.value === 'number' && p.value > 999 ? `$${(p.value / 1000).toFixed(1)}k` : p.value}</span>
        </div>
      ))}
    </div>
  );
  ...
  if (loading || !data || !stats) {
    return (
      <div className="flex flex-col items-center justify-center h-screen space-y-4">
        ...
        <p className="text-sm">Fetching institution data...</p>
      </div>
    );
  }
    ...
    <div className="w-full">
      ...
      <div className="relative z-10 max-w-[1700px] mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className={`text-xs ${isDark ? '' : ''} mt-0.5`}>{user?.role} Dashboard · {format(new Date(), 'MMMM yyyy')}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 rounded-xl border ${isDark ? 'border-white/8  hover:text-white' : 'border-slate-200  hover:'} transition-all text-sm`}
              style={{ background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }}
            >
              <Search className="w-3.5 h-3.5" />
              <span className="hidden sm:block text-xs">Search...</span>
              <kbd className={`hidden sm:flex items-center gap-0.5 text-[10px] ${isDark ? ' border-white/8' : ' border-slate-200'} border rounded px-1 py-0.5`}>⌘K</kbd>
            </button>

            {/* Refresh */}
            <button onClick={handleRefresh}
              className={`w-9 h-9 rounded-xl flex items-center justify-center border ${isDark ? 'border-white/8  hover:text-white' : 'border-slate-200  hover:'} transition-all`}
              style={{ background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }}>
        ...
        {user?.role === 'Admin' && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'Dean\'s List', value: '12', icon: <Star className="w-3.5 h-3.5" />, sub: 'students' },
                { label: 'Overdue Fees', value: '3', icon: <AlertCircle className="w-3.5 h-3.5" />, sub: 'accounts' },
                { label: 'Events This Month', value: '7', icon: <Calendar className="w-3.5 h-3.5" />, sub: 'scheduled' },
                { label: 'Avg CGPA', value: (data.students.reduce((s, st) => s + st.cgpa, 0) / data.students.length).toFixed(2), icon: <GraduationCap className="w-3.5 h-3.5" />, sub: 'institution-wide' },
              ].map((kpi, i) => (
                <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 + i * 0.05 }}
                  ...
                  {kpi.icon}
                  <div>
                    <p className={`text-base font-black ${isDark ? 'text-white' : ''} leading-none`}>{kpi.value}</p>
                    <p className={`text-[10px] ${isDark ? '' : ''} mt-0.5`}>{kpi.label}</p>
                  </div>
                </motion.div>
              ))}
            </div>
                  ...
                  <div className="flex items-center gap-1">
                    {(['7d', '30d', '90d'] as const).map(r => (
                      <button key={r} onClick={() => setChartRange(r)}
                        className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${chartRange === r ? 'bg-indigo-600 text-white' : ' hover:text-white hover:bg-white/5'}`}>
                        {r}
                      </button>
                    ))}
                  </div>
              ...
              >
                <div className="flex items-center gap-4 mb-4">
                  {[{ dot: '#6366f1', label: 'Present' }, { dot: '#ef4444', label: 'Absent' }, { dot: '#f59e0b', label: 'Late' }].map(l => (
                    <span key={l.label} className={`flex items-center gap-1.5 text-xs ${isDark ? '' : ''}`}>
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {feeStatusData.map(item => (
                    <div key={item.name} className={`flex items-center gap-2 p-2 rounded-lg`} style={{ background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }}>
                      ...
                      <div className="min-w-0">
                        <p className={`text-[10px] ${isDark ? '' : ''}`}>{item.name}</p>
                        <p className={`text-sm font-bold ${isDark ? 'text-white' : ''}`}>{item.value}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <GlassCard title="Weekly Attendance Heatmap" subtitle="Last 5 weeks" className="lg:col-span-2">
                <div className="grid grid-cols-5 gap-1.5">
                  {['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map(d => (
                    <p key={d} className={`text-center text-[10px] ${isDark ? '' : ''} font-medium mb-1`}>{d}</p>
                  ))}
                  {weeklyHeatmapData.map((cell, i) => {
                    ...
                    return (
                      ...
                      >
                        <span className={`text-[10px] font-bold ${isDark ? 'text-white/60 group-hover:text-white' : '/40 group-hover:'} transition-colors`}>{cell.value}%</span>
                      </motion.div>
                    );
                  })}
                </div>
                <div className="flex items-center justify-end gap-2 mt-3">
                  <span className={`text-[10px] ${isDark ? '' : ''}`}>Low</span>
                  ...
                  <span className={`text-[10px] ${isDark ? '' : ''}`}>High</span>
                </div>
              </GlassCard>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-5">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-xl ${isDark ? 'bg-amber-500/15' : 'bg-amber-100'} flex items-center justify-center`}>
                      <Trophy className={`w-4 h-4 ${isDark ? '' : ''}`} />
                    </div>
                    <div>
                      <h3 className={`text-base font-bold ${isDark ? 'text-white' : ''}`}>Academic Leaders</h3>
                      <p className={`text-xs ${isDark ? '' : ''}`}>Top performing students this semester</p>
                    </div>
                  </div>
                  <button className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${isDark ? 'border-white/8  hover:text-white hover:border-white/15' : 'border-slate-200  hover: hover:border-slate-300'} text-xs font-medium transition-all`}>
              ...
              <div className="space-y-4">
                <GlassCard title="Quick Actions">
                  <div className="grid grid-cols-2 gap-2">
                  {[
                    { icon: <UserCircle className="w-4 h-4" />, label: 'Add Student', bg: isDark ? 'bg-indigo-500/20' : 'bg-indigo-50', text: isDark ? '' : '', to: '/students' },
                    { icon: <Wallet className="w-4 h-4" />, label: 'Record Fee', bg: isDark ? 'bg-emerald-500/20' : 'bg-emerald-50', text: isDark ? '' : '', to: '/fees' },
                    { icon: <Calendar className="w-4 h-4" />, label: 'Schedule', bg: isDark ? 'bg-amber-500/20' : 'bg-amber-50', text: isDark ? '' : '', to: '/timetable' },
                    { icon: <Mail className="w-4 h-4" />, label: 'Send Notice', bg: isDark ? 'bg-rose-500/20' : 'bg-rose-50', text: isDark ? '' : '', to: '/notices' },
                    { icon: <FileText className="w-4 h-4" />, label: 'Reports', bg: isDark ? 'bg-blue-500/20' : 'bg-blue-50', text: isDark ? '' : '', to: '/teacher-analytics' },
                    { icon: <Settings className="w-4 h-4" />, label: 'Settings', bg: isDark ? 'bg-slate-500/20' : 'bg-slate-50', text: isDark ? '' : '', to: '/settings' },
                  ].map((action, i) => (
                    <motion.button
                      key={i}
                      ...
                      className={`flex flex-col items-center gap-2 p-3.5 rounded-xl border ${isDark ? 'border-white/6 hover:border-white/15' : 'border-slate-100 hover:border-slate-200'} transition-all w-full`}
                        style={{ background: isDark ? 'rgba(255,255,255,0.025)' : 'rgba(255,255,255,1)' }}
                      >
                        <div className={`w-9 h-9 rounded-xl ${action.bg} flex items-center justify-center`}>
                          <span className={action.text}>{action.icon}</span>
                        </div>
                        <span className={`text-[11px] font-medium ${isDark ? '' : ''} text-center`}>{action.label}</span>
                      </motion.button>
                    ))}
                  </div>
                </GlassCard>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className={`flex items-center justify-between p-4 rounded-xl ${isDark ? 'bg-white/5 border-white/8' : 'bg-slate-50 border-slate-100'} border hover:border-indigo-500/30 transition-all cursor-pointer group`}>
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-lg ${isDark ? 'bg-indigo-500/10  border-indigo-500/20' : 'bg-indigo-50  border-indigo-100'} flex flex-col items-center justify-center border`}>
                          <Clock className="w-4 h-4" />
                          <span className="text-[10px] font-bold mt-1">LOG</span>
                        </div>
                        <div>
                          <h4 className={`font-bold ${isDark ? 'text-white' : ''} group-hover: dark:group-hover: transition-colors`}>{session.subject}</h4>
                          <p className="text-xs">{session.time} · {session.room}</p>
                        </div>
                      </div>
                      <span className={`px-3 py-1 rounded-full ${isDark ? 'bg-indigo-500/10  border-indigo-500/20' : 'bg-indigo-50  border-indigo-100'} text-[10px] font-bold border`}>
                        {session.type}
                      </span>
                    </div>
                  ))}
                  ...
                  {data.students.slice(0, 5).map((student, i) => (
                    <div key={i} className={`flex items-center justify-between p-3 rounded-xl ${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'} transition-all cursor-pointer`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className={`text-sm font-medium ${isDark ? 'text-white' : ''}`}>{student.name}</p>
                          <p className={`text-[10px] ${isDark ? '' : ''}`}>Unit 3 Assignment · 2h ago</p>
                        </div>
                      </div>
                      <button className={`px-3 py-1 rounded-lg ${isDark ? 'bg-emerald-500/10  border-emerald-500/20' : 'bg-emerald-50  border-emerald-100'} text-xs font-bold border hover:bg-emerald-500 hover:text-white transition-all`}>
                        Grade
                      </button>
                    </div>
                  ))}
                </div>
                <button className={`w-full mt-4 py-2 text-xs font-bold ${isDark ? ' hover:text-white' : ' hover:'} transition-colors`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div key={i} className={`p-4 rounded-xl ${isDark ? 'bg-white/5 border-white/8' : 'bg-white border-slate-100 shadow-sm'} border hover:border-indigo-500/30 transition-all group overflow-hidden relative`}>
                          <div className="relative z-10">
                            <div className="flex justify-between items-start mb-3">
                              <div className={`w-10 h-10 rounded-lg ${isDark ? 'bg-indigo-500/10 ' : 'bg-indigo-50 '} flex items-center justify-center`}>
                                <BookMarked className="w-5 h-5" />
                              </div>
                              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${progress === 100 ? (isDark ? 'bg-emerald-500/10 ' : 'bg-emerald-50 ') : (isDark ? 'bg-amber-500/10 ' : 'bg-amber-50 ')}`}>
                                {progress === 100 ? 'Completed' : progress > 0 ? 'Submitted' : 'Pending'}
                              </span>
                            </div>
                            <h4 className={`font-bold ${isDark ? 'text-white' : ''} group-hover: dark:group-hover: transition-colors truncate`}>{assign.title}</h4>
                            <p className={`text-xs ${isDark ? '' : ''} mb-4`}>{assign.subject} · {assign.courseCode}</p>

                            <div className="space-y-1.5">
                              <div className="flex justify-between text-[10px]">
                                <span className={` font-medium`}>Status</span>
                                <span className={`${isDark ? 'text-white' : ''} font-bold`}>{progress}%</span>
                              </div>
              ...
              <div className="space-y-6">
                      ...
                      <div key={i} className={`p-4 rounded-xl ${isDark ? 'bg-white/5 border-white/8' : 'bg-white border-slate-100 shadow-sm'} border flex items-center justify-between group hover:border-indigo-500/30 transition-all cursor-pointer`}>
                        <div className="flex items-center gap-4">
                          <div className="text-center min-w-[50px]">
                            <p className={`text-sm font-black ${isDark ? 'text-white' : ''}`}>{cls.time?.split(' ')[0] || '--'}</p>
                            <p className={`text-[10px] ${isDark ? '' : ''} font-bold `}>{cls.time?.split(' ')[1] || '--'}</p>
                          </div>
                          <div className={`w-px h-8 ${isDark ? 'bg-white/10' : 'bg-slate-100'}`} />
                          <div>
                            <h4 className={`font-bold ${isDark ? 'text-white' : ''} group-hover: dark:group-hover: transition-colors`}>{cls.subject}</h4>
                            <p className={`text-xs ${isDark ? '' : ''} truncate max-w-[120px]`}>{cls.instructor || 'Staff'} · {cls.room}</p>
                          </div>
                        </div>
                        <ChevronRight className={`w-4 h-4 ${isDark ? '' : ''} group-hover: dark:group-hover:text-white transition-all transform group-hover:translate-x-1`} />
                      </div>
                    ))}
                  </div>
                  <button className={`w-full mt-4 py-2.5 rounded-xl ${isDark ? 'bg-white/5 border-white/5  hover:text-white' : 'bg-slate-50 border-slate-100  hover:'} border text-xs font-bold transition-all`}>
                    View Full Timetable
                  </button>
                </GlassCard>
                ...
                <GlassCard title="Quick Actions">
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { icon: <BookOpen className="w-4 h-4" />, label: 'Courses', color: ' dark:', bg: 'bg-indigo-50 dark:bg-indigo-500/10' },
                      { icon: <ClipboardList className="w-4 h-4" />, label: 'Tasks', color: ' dark:', bg: 'bg-emerald-50 dark:bg-emerald-500/10' },
                      { icon: <Library className="w-4 h-4" />, label: 'Library', color: ' dark:', bg: 'bg-amber-50 dark:bg-amber-500/10' },
                      { icon: <Clock className="w-4 h-4" />, label: 'Exams', color: ' dark:', bg: 'bg-rose-50 dark:bg-rose-500/10' },
                      { icon: <Activity className="w-4 h-4" />, label: 'Grades', color: ' dark:', bg: 'bg-indigo-50 dark:bg-indigo-500/10' },
                      { icon: <Mail className="w-4 h-4" />, label: 'Help', color: ' dark:', bg: 'bg-slate-50 dark:bg-slate-500/10' },
                    ].map((action, i) => (
                      <button key={i} className={`flex flex-col items-center gap-2 p-3 rounded-xl border ${isDark ? 'bg-white/5 border-white/5 hover:border-indigo-500/20' : 'bg-white border-slate-100 hover:border-indigo-100 shadow-sm'} transition-all group active:scale-95`}>
                        ...
                        <span className={`text-[10px] font-bold ${isDark ? '' : ''} group-hover: dark:group-hover: transition-colors  tracking-wider`}>{action.label}</span>
                      </button>
                    ))}
                  </div>
                </GlassCard>
                ...
                <GlassCard title="Recent Activity" subtitle="Your latest interactions">
                  <div className="space-y-4">
                    {[
                      { icon: <CheckCircle2 className="w-3.5 h-3.5" />, title: 'Assignment Submitted', desc: 'Database Schema Project', time: '2 hours ago', color: ' dark:', bg: 'bg-emerald-50 dark:bg-emerald-500/10' },
                      { icon: <Star className="w-3.5 h-3.5" />, title: 'Grade Received', desc: 'A- in System Design', time: 'Yesterday', color: ' dark:', bg: 'bg-amber-50 dark:bg-amber-500/10' },
                      { icon: <Bell className="w-3.5 h-3.5" />, title: 'New Notice', desc: 'Term Exams Canceled', time: '2 days ago', color: ' dark:', bg: 'bg-indigo-50 dark:bg-indigo-500/10' },
                    ].map((item, i) => (
                      <div key={i} className="flex gap-3 relative group">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={`text-xs font-bold ${isDark ? 'text-white' : ''} group-hover: dark:group-hover: transition-colors  tracking-wider`}>{item.title}</p>
                          <p className={`text-[11px] ${isDark ? '' : ''} truncate mt-0.5`}>{item.desc}</p>
                          <p className={`text-[9px] ${isDark ? '' : ''} mt-1`}>{item.time}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-700 relative overflow-hidden group shadow-xl">
                  <div className="relative z-10 flex items-center justify-between">
                    <div>
                      <p className="text-[10px] font-black   tracking-[0.2em] mb-1">Weekly Goal</p>
        ...
        <div className="flex items-center justify-between pt-2 pb-8">
          <p className={`text-[11px] ${isDark ? '' : ''}`}>Last updated: {format(new Date(), 'h:mm a · MMM d, yyyy')}</p>
          <div className="flex items-center gap-3">
            <button className={`text-[11px] ${isDark ? '' : ''} hover: transition-colors`}>Privacy</button>
            <span className={isDark ?"" :""}>·</span>
            <button className={`text-[11px] ${isDark ? '' : ''} hover: transition-colors`}>Help</button>
            <span className={isDark ?"" :""}>·</span>
            <button className={`text-[11px] ${isDark ? '' : ''} hover: transition-colors`}>v2.1.0</button>
          </div>
        </div>
      </div>
    </div>

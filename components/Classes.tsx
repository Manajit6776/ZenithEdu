...
const ClassCard: React.FC<ClassCardProps> = ({
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className={`bg-white/40 dark:bg-slate-900 rounded-3xl overflow-hidden group transition-all duration-500 border p-1 relative shadow-xl shadow-rose-200/5 ${classItem.status === 'Live'
        ? 'border-rose-500/30 ring-1 ring-rose-500/20'
        : 'border-rose-200/20 dark:border-slate-800'
        }`}
    >
      <div className="p-6">
        <div className="flex justify-between items-start mb-6">
          <div className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border shadow-sm ${statusStyles[classItem.status]}`}>
            {classItem.status === 'Live' && <span className="w-1.5 h-1.5 bg-rose-500 rounded-full"></span>}
          ...
          <div className="flex gap-2">
            <motion.button
              ...
              className="p-2.5 rounded-xl bg-rose-100 dark:bg-slate-800 text-rose-600 dark:text-slate-400 hover:text-rose-800 border border-rose-200/30 dark:border-slate-700 transition-all"
        ...
        <div className="space-y-4 mb-8">
          <div className="flex items-center gap-3 text-xs">
            <div className="w-9 h-9 rounded-2xl bg-rose-600 text-white flex items-center justify-center font-black shadow-lg shadow-rose-200/50">
              {classItem.instructor.charAt(0)}
            </div>
            <span className="font-black text-rose-800 dark:text-slate-300 uppercase text-[10px] tracking-widest">{classItem.instructor}</span>
          </div>
        ...
        <motion.button
          ...
          className={`w-full py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${classItem.status === 'Live'
            ? 'bg-rose-600 hover:bg-rose-500 text-white shadow-xl shadow-rose-200/50'
            : hasReminder
              ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200/50'
              : 'bg-rose-50 dark:bg-white/5 text-rose-600 dark:text-slate-300 hover:bg-rose-600 hover:text-white border border-rose-200/50'
            }`}
        >
...
export const Classes: React.FC = () => {
  console.log('Classes component rendering...');
  const { t } = useLanguage();
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] space-y-4">
        ...
        <p className="text-rose-800 dark:text-slate-500 animate-pulse font-black uppercase text-xs tracking-widest">Entering Virtual Halls</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 pb-10 custom-scrollbar relative">
      ...
      >
        <div>
          <h1 className="text-3xl font-black text-rose-800 dark:text-white uppercase tracking-tight flex items-center gap-4">
            <span className="p-3 rounded-2xl bg-rose-600/10 text-rose-600 border border-rose-500/20 shadow-lg shadow-rose-200/20">
              <Video className="w-8 h-8" />
            </span>
            {t('liveClasses')}
          </h1>
          <p className="text-slate-600 dark:text-slate-400 font-bold text-xs uppercase tracking-widest mt-2 flex items-center gap-2">
        ...
        <div className="flex gap-4">
          ...
          {(isTeacher() || isAdmin()) && (
            <motion.button
              ...
              className="flex items-center gap-3 px-8 py-3.5 rounded-2xl font-black bg-rose-600 text-white hover:bg-rose-500 transition-all shadow-xl shadow-rose-200/50 uppercase text-xs tracking-widest"
            >
      ...
      <motion.div
        ...
        className="bg-white/40 dark:bg-slate-900 p-5 rounded-3xl border border-rose-200/30 dark:border-slate-700 flex flex-col lg:flex-row gap-5 shadow-xl shadow-rose-200/5 relative z-20"
      >
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3.5 bg-white/50 dark:bg-slate-800 rounded-2xl border border-rose-200/30 dark:border-slate-700 text-rose-900 dark:text-white placeholder:text-rose-200 dark:placeholder-slate-600 outline-none focus:ring-4 focus:ring-rose-500/10 transition-all font-bold"
            value={searchTerm}
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative">
            <button
              ...
              className="flex items-center gap-4 px-6 py-3.5 bg-white/50 dark:bg-slate-800 border border-rose-200/30 dark:border-slate-700 rounded-2xl hover:bg-rose-50 dark:hover:bg-slate-700 transition-all text-rose-800 dark:text-slate-300 font-black text-xs uppercase tracking-widest shadow-sm"
            >
      ...
      {showForm && (
        <div className="fixed inset-0 bg-rose-900/40 backdrop-blur-md flex items-center justify-center p-4 z-[100] animate-in fade-in duration-300">
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-white/95 dark:bg-slate-900 rounded-[2.5rem] p-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-rose-200/50 dark:border-slate-700 shadow-2xl relative custom-scrollbar"
          >
            ...
            <form onSubmit={(e) => {
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full px-6 py-4 bg-rose-50 dark:bg-slate-800 border border-rose-100 dark:border-slate-700 rounded-2xl text-rose-900 dark:text-white outline-none focus:ring-4 focus:ring-rose-500/10 transition-all font-bold appearance-none cursor-pointer"
                    >
                ...
                <div className="space-y-3 md:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-14 pr-6 py-4 bg-rose-50 dark:bg-slate-800 border border-rose-100 dark:border-slate-700 rounded-2xl text-rose-900 dark:text-white placeholder:text-rose-200 dark:placeholder-slate-600 outline-none focus:ring-4 focus:ring-rose-500/10 transition-all font-bold"
                    />
                  </div>
                </div>
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-6 py-4 bg-rose-50 dark:bg-slate-800 border border-rose-100 dark:border-slate-700 rounded-2xl text-rose-900 dark:text-white outline-none focus:ring-4 focus:ring-rose-500/10 transition-all font-bold"
                    />
              ...
              <div className="flex gap-6 pt-6 border-t border-rose-50 dark:border-slate-800">
                ...
                <button
                  type="submit"
                  className="flex-1 py-4 rounded-2xl bg-rose-600 hover:bg-rose-500 text-white font-black text-[10px] uppercase tracking-widest transition-all shadow-xl shadow-rose-200/50"
                >

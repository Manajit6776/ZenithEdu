...
const colorMap: Record<string, { bg: string; border: string; icon: string; badge: string; glow: string; dot: string }> = {
  rose:   { bg: 'bg-rose-500/10 dark:bg-indigo-600/15',   border: 'border-rose-100 dark:border-indigo-500/20',   icon: 'text-rose-600 dark:text-indigo-400',   badge: 'bg-rose-100 dark:bg-indigo-500/20 text-rose-700 dark:text-slate-300',   glow: 'from-rose-500/10',   dot: 'bg-rose-500' },
  blue:   { bg: 'bg-indigo-500/10 dark:bg-indigo-600/15', border: 'border-indigo-100 dark:border-indigo-500/20', icon: 'text-indigo-600 dark:text-indigo-400', badge: 'bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-slate-300', glow: 'from-indigo-500/10', dot: 'bg-indigo-500' },
  amber:  { bg: 'bg-amber-500/10 dark:bg-indigo-600/15',  border: 'border-amber-100 dark:border-indigo-500/20',  icon: 'text-amber-600 dark:text-indigo-400',  badge: 'bg-rose-100 dark:bg-indigo-500/20 text-rose-700 dark:text-slate-300',   glow: 'from-amber-500/10', dot: 'bg-amber-500' },
  pink:   { bg: 'bg-rose-500/10 dark:bg-indigo-600/15',   border: 'border-rose-100 dark:border-indigo-500/20',   icon: 'text-rose-600 dark:text-indigo-400',   badge: 'bg-rose-100 dark:bg-indigo-500/20 text-rose-700 dark:text-slate-300',   glow: 'from-rose-500/10',   dot: 'bg-rose-500' },
};
...
export function FeaturesPage() {
  ...
  return (
    <div className="min-h-screen bg-amber-50 dark:bg-[#050508] text-rose-900 dark:text-indigo-400">
      ...
      <section className="relative pt-20 pb-16 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 text-center">
          ...
          <motion.h1
            ...
            className="text-5xl sm:text-6xl lg:text-7xl font-black tracking-tight text-rose-950 dark:text-slate-300 mb-8 leading-[1.05]"
          >
          ...
          <motion.p
            ...
            className="text-lg text-rose-950/60 dark:text-indigo-400/60 max-w-2xl mx-auto leading-relaxed font-black uppercase tracking-tight"
          >
            ZenithEdu synchronizes 30+ enterprise-grade modules into one unified ecosystem — architected to accelerate your institution's evolution.
          </motion.p>
          ...
          >
            {[
              { value: '30+', label: 'DYNAMIC MODULES', color: 'text-rose-600 dark:text-indigo-500' },
              { value: '99.9%', label: 'UPTIME SLA', color: 'text-amber-600 dark:text-indigo-400' },
              { value: '<2s', label: 'OPTIMAL SPEED', color: 'text-emerald-600 dark:text-indigo-500' },
              { value: '24/7', label: 'ELITE SUPPORT', color: 'text-rose-600 dark:text-indigo-400' },
            ].map(s => (
              <div key={s.label} className="flex flex-col items-center gap-1 bg-amber-50/40 dark:bg-white/5 border border-rose-100 dark:border-indigo-500/20 px-6 py-3 rounded-2xl backdrop-blur-md">
                <span className={`text-2xl font-black ${s.color}`}>{s.value}</span>
                <span className="text-[9px] text-rose-900/40 dark:text-indigo-400/40 font-black uppercase tracking-widest">{s.label}</span>
              </div>
            ))}
          </motion.div>
        </div>
            ...
            <button
              ...
              className={`px-8 py-3 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${
                active === f.category
                  ...
                  : 'text-rose-800/40 dark:text-indigo-400/40 hover:text-rose-600 dark:hover:text-blue-400'
              }`}
            >
              {f.category}
            </button>
          ...
          {activeSection.items.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-lg font-bold text-rose-950 dark:text-slate-300 mb-2">{item.title}</h3>
                <p className="text-sm text-rose-900/60 dark:text-indigo-400/60 leading-relaxed mb-5">{item.desc}</p>

                <ul className="space-y-2">
                  {item.highlights.map(h => (
                    <li key={h} className="flex items-center gap-2 text-xs text-rose-900/60 dark:text-indigo-400/60">
      ...
      <section className="relative bg-gradient-to-br from-rose-600 via-rose-500 to-pink-600 dark:from-blue-700 dark:via-blue-800 dark:to-indigo-900 text-white py-24 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-rose-100/80 dark:text-indigo-200/80 mb-12 text-lg font-medium">Join 50+ elite institutions already redefining the boundaries of education.</p>
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <Link to="/login" className="px-10 py-4 bg-white text-rose-600 dark:text-indigo-600 font-black rounded-2xl hover:bg-rose-50 dark:hover:bg-blue-50 transition-all shadow-2xl shadow-black/20 uppercase tracking-widest text-[11px]">

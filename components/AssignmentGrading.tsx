...
export const AssignmentGrading: React.FC<AssignmentGradingProps> = ({ assignmentId, onClose, onGradeComplete }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return <span className="px-3 py-1 text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-100 rounded-full">{status}</span>;
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] pl-64 p-4">
      <div className="bg-white/95 dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col overflow-hidden border border-rose-100 dark:border-white/5">
        {/* Header */}
        <div className="bg-gradient-to-r from-rose-600 via-pink-600 to-rose-500 dark:from-indigo-600 dark:via-violet-600 dark:to-indigo-500 text-white p-5">

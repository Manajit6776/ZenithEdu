...
const InfoItem: React.FC<{ icon: any; label: string; value: string }> = ({ icon: Icon, label, value }) => (
  <div className="space-y-2">
    <div className="flex items-center gap-2">
      <Icon className="w-4 h-4 text-rose-400 dark:text-slate-500" />
      <span className="text-[10px] font-black text-rose-800/40 dark:text-slate-700/40 uppercase tracking-widest">{label}</span>
    </div>
    <p className="text-sm font-black text-rose-900 dark:text-slate-300 uppercase tracking-tight ml-6">{value}</p>
  </div>
);
...
const FormField: React.FC<{ label: string; required?: boolean; children: React.ReactNode }> = ({ label, required, children }) => (
  <div className="space-y-2">
    <label className="block text-[10px] font-black text-rose-800 dark:text-slate-500/50 uppercase tracking-widest ml-1">
      {label} {required && <span className="text-rose-500 dark:text-slate-500">*</span>}
    </label>
    {children}
  </div>
);
    ...
    <div className="p-4 sm:p-5 max-w-full space-y-5">
      ...
      {(filterDay !== 'All' || filterType !== 'All' || searchTerm) && (
        <div className="flex flex-wrap gap-3 items-center ml-2">
          <span className="text-[10px] font-black text-rose-800/40 dark:text-slate-200/40 uppercase tracking-[0.2em]">Active Filters</span>
          ...
          <div className="ml-auto text-[10px] font-black text-rose-800/40 dark:text-slate-200/40 uppercase tracking-widest">{filteredTimetable.length} Class{filteredTimetable.length !== 1 ? 'es' : ''}</div>
        </div>
      )}
...
const Chip: React.FC<{ label: string; onRemove: () => void; color: 'rose' | 'amber' | 'blue' }> = ({ label, onRemove, color }) => {
  const colorClasses = {
    ...
    blue: 'bg-blue-100/50 text-blue-700 border-blue-200/50 dark:bg-indigo-950/20 dark:text-indigo-400 dark:border-indigo-900/30',
  };
  return (

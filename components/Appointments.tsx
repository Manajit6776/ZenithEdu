...
export const Appointments: React.FC = () => {
  ...
  const getStatusColor = (status: Appointment['status']) => {
    switch (status) {
      ...
      default: return 'bg-slate-100/50 text-slate-900 dark:bg-slate-800/50 dark:text-slate-100';
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] space-y-4">
        ...
        <p className="text-slate-500 dark:text-slate-400 animate-pulse font-black uppercase text-xs tracking-widest">Loading Appointments</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black  text-rose-800 dark:text-white tracking-tight">Appointments</h1>
          <p className="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">
            {user?.role === 'Student'
        ...
        <div className="flex gap-3 w-full md:w-auto">
          {user?.role === 'Student' && (
            <button
              ...
              className="px-6 py-2.5 bg-slate-900 dark:bg-indigo-600 hover:bg-slate-800 dark:hover:bg-indigo-500 text-white font-black rounded-xl transition-all shadow-lg shadow-slate-200/50 dark:shadow-indigo-500/20 uppercase text-xs tracking-widest flex items-center gap-2"
            >
              {t('bookAppointment')}
            </button>
          )}

          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-1 flex">
            <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm group hover:border-amber-400 dark:hover:border-amber-500/30 transition-all">
          <div className="flex justify-between items-center text-left">
            <div>
              <p className="text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1.5">Pending Request</p>
              <p className="text-2xl font-black text-amber-500 dark:text-amber-400">
                {appointments.filter(a => a.status === 'Pending').length}
              </p>
            </div>
            <div className="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-500 border border-amber-100 dark:border-amber-500/10 group-hover:scale-110 transition-transform">
              <Clock className="w-5 h-5" />
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm group hover:border-indigo-400 dark:hover:border-indigo-500/30 transition-all">
          <div className="flex justify-between items-center text-left">
            <div>
              <p className="text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1.5">Approved Sync</p>
              <p className="text-2xl font-black text-indigo-600 dark:text-indigo-400">
                {appointments.filter(a => a.status === 'Approved').length}
              </p>
            </div>
            <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-500 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-500/10 group-hover:scale-110 transition-transform">
              <Check className="w-5 h-5" />
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm group hover:border-emerald-400 dark:hover:border-emerald-500/30 transition-all">
          <div className="flex justify-between items-center text-left">
            <div>
              <p className="text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1.5">Compl. Missions</p>
              <p className="text-2xl font-black text-emerald-500 dark:text-emerald-400">
                {appointments.filter(a => a.status === 'Completed').length}
              </p>
            </div>
            <div className="w-12 h-12 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center text-emerald-500 border border-emerald-100 dark:border-emerald-500/10 group-hover:scale-110 transition-transform">
              <CheckCircle className="w-5 h-5" />
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm group hover:border-slate-400 dark:hover:border-slate-500/30 transition-all">
          <div className="flex justify-between items-center text-left">
            <div>
              <p className="text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1.5">Total Docket</p>
              <p className="text-2xl font-black text-slate-900 dark:text-white">
                {appointments.length}
              </p>
            </div>
            <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 group-hover:scale-110 transition-transform">
      ...
      {/* Search and Filter */}
      {/* Search and Filter */}
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 p-4 md:p-5 shadow-sm">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 dark:focus:ring-indigo-500/30 transition-all font-bold text-xs uppercase tracking-widest"
            />
          </div>
          <select
            value={filterStatus}
            onChange={(e) => setFilterStatus(e.target.value)}
            className="px-6 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 dark:focus:ring-indigo-500/30 transition-all font-black text-[10px] uppercase tracking-widest appearance-none cursor-pointer min-w-[160px]"
          >
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {(getTodayAppointments().length > 0 ? getTodayAppointments() : filteredAppointments).length === 0 ? (
              ...
              (getTodayAppointments().length > 0 ? getTodayAppointments() : filteredAppointments).map((appointment) => (
                <div
                  key={appointment.id}
                  className={`bg-white dark:bg-slate-900 p-5 rounded-2xl flex items-start gap-5 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all border border-slate-200 dark:border-slate-800 shadow-sm group`}
                >
                  <div className="flex flex-col items-center min-w-[70px] bg-indigo-50 dark:bg-slate-800 p-3 rounded-xl ring-1 ring-indigo-100 dark:ring-slate-700 group-hover:bg-indigo-100 dark:group-hover:bg-slate-700 transition-colors">
                    <span className="text-[10px] font-black text-indigo-700/60 dark:text-slate-500 uppercase tracking-tighter">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="text-lg font-black text-slate-900 dark:text-white group-hover:text-indigo-700 dark:group-hover:text-indigo-400 transition-colors">{appointment.subject}</h4>

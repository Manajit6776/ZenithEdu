...
const PRIORITY_CONFIG: Record<Priority, {
  ...
  High:   { icon: AlertTriangle, label: 'High',   bar: 'bg-red-500',    badge: 'bg-red-500/10 text-red-400 border-red-500/25',    badgeDark: '',  glow: 'hover:shadow-red-900/20'   },
  Medium: { icon: Info,          label: 'Medium', bar: 'bg-amber-500',  badge: 'bg-amber-500/10 text-amber-400 border-amber-500/25', badgeDark: '', glow: 'hover:shadow-amber-900/20' },
  Low:    { icon: CheckCircle,   label: 'Low',    bar: 'bg-blue-500',   badge: 'bg-blue-500/10 text-blue-400 border-blue-500/25',   badgeDark: '', glow: 'hover:shadow-blue-900/20'  },
};
      ...
      <div className="pl-4 pr-4 pt-4 pb-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-2.5">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {notice.pinned && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/25">
          ...
          {canEdit && (
            <div className="relative flex-shrink-0" ref={menuRef} onClick={e => e.stopPropagation()}>
              <button onClick={() => setShowMenu(!showMenu)} className="p-1 rounded-md hover:bg-white/10 text-slate-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100">
              ...
              {showMenu && (
                <div className="absolute right-0 top-full mt-1 w-44 rounded-lg bg-slate-900/95 border border-white/10 shadow-2xl z-30 backdrop-blur-sm">
                  <div className="py-1">
                    <button onClick={() => { onView(notice); setShowMenu(false); }} className="w-full px-3 py-2 text-left text-sm text-slate-300 hover:bg-white/5 flex items-center gap-2 transition-colors">
                      <Eye className="w-3.5 h-3.5" /> View Notice
                    </button>
                    {onEdit && (
                      <button onClick={() => { onEdit(notice); setShowMenu(false); }} className="w-full px-3 py-2 text-left text-sm text-slate-300 hover:bg-white/5 flex items-center gap-2 transition-colors">
                    ...
                    {onDelete && (
                      <>
                        ...
                        <button onClick={() => { onDelete(notice); setShowMenu(false); }} className="w-full px-3 py-2 text-left text-sm text-red-400 hover:bg-red-500/10 flex items-center gap-2 transition-colors">
        ...
        {/* Title */}
        <h3 className="text-[15px] font-black text-rose-800 dark:text-white leading-snug mb-2 pr-1 line-clamp-2">{notice.title}</h3>

        {/* Content preview */}
        <p className="text-sm text-slate-700 dark:text-slate-400 leading-relaxed line-clamp-3 mb-4 font-medium">{notice.content}</p>

        {/* Footer */}
        <div className="flex items-center justify-between text-xs text-slate-500 pt-3 border-t border-white/6">
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className="bg-slate-900/98 border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                {notice.pinned && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-sm font-medium bg-indigo-500/10 text-indigo-400 border border-indigo-500/25">
        ...
        <div className="px-6 py-5">
          <p className="text-[15px] text-slate-300 leading-relaxed whitespace-pre-wrap">{notice.content}</p>

          <div className="mt-5 pt-5 border-t border-white/8 flex items-center justify-between text-sm text-slate-500">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-slate-300 font-medium text-xs">{notice.author}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-xs text-slate-600">Date</p>
              <p className="text-slate-300 text-xs font-medium">{formatFullDate(notice.date)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 pb-6 flex gap-2.5">
          {canEdit && onEdit && (
            <button onClick={onEdit} className="flex-1 py-2.5 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-slate-300 text-sm font-medium flex items-center justify-center gap-2 transition-colors">
              <Edit className="w-4 h-4" /> Edit
            </button>
          )}
          {canEdit && onDelete && (
            <button onClick={onDelete} className="flex-1 py-2.5 rounded-xl border border-red-500/20 bg-red-500/10 hover:bg-red-500/20 text-red-400 text-sm font-medium flex items-center justify-center gap-2 transition-colors">
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4 z-50" onClick={onClose}>
      <div className="bg-slate-900/98 border border-white/10 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-xl shadow-2xl max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 border-b border-white/8 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold text-white">New Notice</h2>
            <p className="text-xs text-slate-500 mt-0.5">Post an announcement to the board</p>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          {/* Title + AI Generate */}
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">Topic / Title <span className="text-indigo-400">*</span></label>
            <div className="flex gap-2">
              <input
                ...
                className="glass-input flex-1 px-3 py-2.5 rounded-xl text-sm placeholder:text-slate-600 focus:ring-2 focus:ring-indigo-500/40 outline-none"
              />
              <button
            ...
            <p className="mt-1.5 text-[11px] text-slate-600">Press ⌘ + Enter to generate with AI</p>
          </div>
          ...
          {/* Content */}
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-1.5">Content <span className="text-indigo-400">*</span></label>
            <textarea
              value={draft}
              onChange={e => setDraft(e.target.value)}
              rows={5}
              className="glass-input w-full px-3 py-2.5 rounded-xl text-sm placeholder:text-slate-600 focus:ring-2 focus:ring-indigo-500/40 outline-none resize-none leading-relaxed"
              placeholder={isGenerating ? '' : 'Write notice content here, or use AI Draft above…'}
            />
            <p className="mt-1 text-right text-[11px] text-slate-600">{draft.length} chars</p>
          </div>
              ...
              {(['High', 'Medium', 'Low'] as Priority[]).map(p => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-sm font-medium border transition-all duration-150 flex items-center justify-center gap-1.5 ${active ? pc.badge : 'bg-white/5 text-slate-400 border-white/10 hover:bg-white/10'}`}
                  >
        ...
        <div className="px-6 pb-6 flex gap-2.5 border-t border-white/8 pt-4">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 text-sm font-medium transition-colors">
            Cancel
          </button>
          <button
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-0 py-2 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-rose-800 dark:text-white tracking-tight">Notice Board</h1>
          <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 font-medium">Important announcements and updates</p>
          {!loading && <div className="mt-2"><StatsBar notices={notices} /></div>}
        </div>

        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={loadNotices} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/8 text-slate-400 hover:text-white transition-colors" title="Refresh">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-white/40 dark:bg-slate-900/80 text-black dark:text-white border border-rose-200/30 dark:border-white/10 rounded-xl px-9 py-2.5 text-sm placeholder:text-slate-500 focus:ring-2 focus:ring-rose-500/40 outline-none font-medium"
          />
          {searchTerm && (
            <button onClick={() => setSearchTerm('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-black dark:hover:text-white transition-colors">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
          ...
          {(['All', 'High', 'Medium', 'Low'] as (Priority | 'All')[]).map(p => {
            ...
            return (
              <button key={p} onClick={() => setFilterPriority(p)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all duration-150 ${active && cfg ? cfg.badge : active ? 'bg-indigo-600 text-white border-indigo-500/50' : 'bg-white/5 text-slate-400 border-white/8 hover:bg-white/10 hover:text-slate-200'}`}>
                {p === 'All' ? 'All' : p}
              </button>
            );
          })}
      ...
      {(searchTerm || filterPriority !== 'All') && !loading && (
        <div className="flex flex-wrap items-center gap-2 -mt-1">
          ...
          <span className="text-xs text-slate-500">of {notices.length} notices</span>
          <button onClick={() => { setSearchTerm(''); setFilterPriority('All'); }} className="text-xs text-slate-500 hover:text-slate-300 underline transition-colors ml-1">Clear filters</button>
        </div>
      )}
          ...
          {pinned.length > 0 && (
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pinned.map(n => (
                  <NoticeCard key={n.id} notice={n} canEdit={canEdit} onView={setViewingNotice} onEdit={() => {}} onDelete={() => handleDelete(n)} isNew={newIds.has(n.id)} />
                ))}
              </div>
            </section>
          )}
            ...
            <section>
              {pinned.length > 0 && (
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">All Notices</span>
      ...
      {showCompose && (
        <ComposeModal onPost={handlePost} onClose={() => setShowCompose(false)} />
      )}
    </div>
  );
};
...
const EmptyState: React.FC<{ hasFilters: boolean; onReset: () => void; onAdd: () => void; canAdd: boolean }> = ({ hasFilters, onReset, onAdd, canAdd }) => (
  <div className="flex flex-col items-center justify-center py-20 text-center">
    ...
    <p className="text-sm text-slate-500 mb-6 max-w-xs">{hasFilters ? 'Try adjusting your search or priority filter.' : 'Keep everyone informed by posting the first announcement.'}</p>
